  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      try {
        ...
        if (!listed.length) {
          const filename = initialActiveFile || codeWorkspace.defaultFilename({ id: chapterId });
          const content = buildDefaultScript(chapterId, chapterTitle);
          await codeWorkspace.createFile(chapterId, filename, content);
